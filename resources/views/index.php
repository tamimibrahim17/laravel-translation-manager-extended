    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\ShuvroRoy\TranslationManager\Controller@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\ShuvroRoy\TranslationManager\Controller@getIndex') ?>';
                }
            });
...
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\ShuvroRoy\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-import" method="POST" action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                ...
                <a href="<?= action('\ShuvroRoy\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">Back</a>
            </form>
        <?php endif; ?>
    </p>
    ...
    <h4>Total: <?= $numTranslations ?>, changed: <?= $numChanged ?></h4>
        <form action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET"  role="form">
            <div class="form-group">
                ...
                <div role="tabpanel" class="tab-pane <?php echo request()->has('all') || request()->has('order') || request()->has('orderBy') || empty(request()->all())  ? 'active' : '' ?>" id="home">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <form action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET" id="orderForm" style="display: none;">
                            ...
                            <?php foreach ($locales as $locale): ?>
                                ...
                                <form action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET" id="orderByForm" style="display: none;">
                                ...
                                <?php foreach ($locales as $locale): ?>
                                    ...
                                    <td>
                                        ...
                                        <form action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@postApprove', [$group]) ?>" method="POST" role="form">
                                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                                    <input type="hidden" name="local" value="<?php echo $locale; ?>">
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\ShuvroRoy\TranslationManager\Controller@postRemoveLocale') ?>" data-confirm="Are you sure to remove this locale and all of data?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
